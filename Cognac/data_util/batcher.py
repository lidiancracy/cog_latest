...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab, stmts):
    ...
    self.enc_stmts = stmts[:config.max_enc_steps]
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list):
    ...
    self.enc_stmts = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.int32)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract, stmts) = input_gen.__next__()  # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, abstract_sentences, self._vocab, stmts) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    while True:
        ...
        try:
            article_text = e['article']  # the article text was saved under the key 'article' in the data files
            abstract_text = e['abstract']  # the abstract text was saved under the key 'abstract' in the data files
            stmts = e['stmts']
        except ValueError:
        ...
        if len(article_text) == 0:
            ...
            yield (article_text, abstract_text, stmts)
